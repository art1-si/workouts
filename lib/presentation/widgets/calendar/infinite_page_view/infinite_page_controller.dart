...
class InfinitePageController extends ScrollController {
  ...
  double? get page {
    ...
    final position = this.position as InfinitePagePosition;
    return position.page;
  }

  /// Returns rounded [page] value.
  int get pageRounded {
    return page?.round() ?? 0;
  }
  ...
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition? oldPosition) {
    return InfinitePagePosition(
      useNegative: initialPage.isNegative,
      physics: physics,
      context: context,
  ...
  void attach(ScrollPosition position) {
    ...
    (position as InfinitePagePosition).viewportFraction = viewportFraction;
  }
}

class InfinitePagePosition extends ScrollPositionWithSingleContext implements PageMetrics {
  InfinitePagePosition({
  ...
  void forcePixels(double value) {
    super.forcePixels(useNegative ? -value : value);
  }
  ...
  late double _pageToUseOnStartup;
  ...
  double getPageFromPixels(double pixels, double viewportDimension) {
    ...
    final actual = (pixels - _initialPageOffset) / (viewportDimension * viewportFraction);
  ...
  void restoreScrollOffset() {
    if (!hasPixels) {
      final value = PageStorage.maybeOf(context.storageContext)?.readState(context.storageContext) as double?;
      if (value != null) {
  ...
  bool applyViewportDimension(double viewportDimension) {
    final oldViewportDimensions = hasViewportDimension ? this.viewportDimension : null;
    ...
    if (oldPixels == null) {
      ...
      page = getPageFromPixels(oldPixels, oldViewportDimensions ?? 1);
    }
  ...
  void absorb(ScrollPosition other) {
    ...
    if (other is! InfinitePageController) {
      return;
    }

    if ((other as InfinitePagePosition)._cachedPage != null) {
